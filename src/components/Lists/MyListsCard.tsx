                ...
                <Link href={path}
                    ...
                    {list.map((list: any, index: any) => (
                        <img
                            key={index}
                            src={
                                list.properties.Poster && list.properties.Poster.url !== null
                                    ? list.properties.Poster.url
                                    : list.properties["Cover Image"] && list.properties["Cover Image"].url !== null
                                        ? list.properties["Cover Image"].url
                                        : "/no-image.png"
                            }
                            alt=""
                    ...
                    </p>
                    {
                        list.map((list: any, index: any) => (
                            <Link href={`https://www.notion.so/${list.id.replace(/-/g, '')}`} target='_blank' key={index} className="inline-block bg-gray-200 rounded-full px-3 py-1 text-xs text-gray-700 mr-2 mb-2">
                                {list.icon.emoji} {list.properties.Name?.title[0]?.plain_text || list.properties.Title?.title[0]?.plain_text}
                            </Link>
                        ))
                    }
                </div>
